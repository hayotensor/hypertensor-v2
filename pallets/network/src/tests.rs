...
use crate::{
  ...
  BaseRewardPerMB,
  SubnetRemovalReason, ProposalBidAmount, BaseSubnetNodeMemoryMB,
  MinSubnetDelegateStakePercentage, MaxSubnetPenaltyCount, 
  MaxSubnetMemoryMB, StakeUnbondingLedger, TotalSubnetMemoryMB,MaxTotalSubnetMemoryMB,
  ...
  LastSubnetRegistrationEpoch, MinSubnetRegistrationFee, MaxSubnetRegistrationFee, SubnetRegistrationFeePeriod,
};
...
fn test_register_subnet_node() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(total_subnet_node_uids - 1, hotkey_subnet_node_id);
...
fn test_update_coldkey() {
  new_test_ext().execute_with(|| {
    ...
    let stake_balance = AccountSubnetStake::<Test>::get(&account(0), subnet_id);
    ...
    let original_unbondings: BTreeMap<u64, u128> = StakeUnbondingLedger::<Test>::get(account(0));
    ...
    let unbondings: BTreeMap<u64, u128> = StakeUnbondingLedger::<Test>::get(account(0));
    let ledger_balance: u128 = unbondings.values().copied().sum();
...
fn test_remove_stake_twice_in_epoch() {
  new_test_ext().execute_with(|| {
    ...
    let _ = Balances::deposit_creating(&account(0), amount*2);
