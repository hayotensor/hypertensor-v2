...
use crate::{
  ...
  TotalSubnets,
  SubnetRewardsSubmission, BaseValidatorReward,
  DelegateStakeRewardsPercentage,
  SubnetPenaltyCount, MaxSubnetNodePenalties, 
  SubnetNodePenalties2, RegistrationSubnetData,
  BaseRewardPerMB,
  ...
  HotkeyOwner, TotalSubnetNodeUids, HotkeySubnetNodeId, SubnetNodeIdHotkey, SubnetNodesData2, SubnetNodeAccount2
};
...
fn build_activated_subnet(subnet_path: Vec<u8>, start: u32, mut end: u32, deposit_amount: u128, amount: u128) {
  ...
  for n in start..end {
    ...
    let subnet_node_data = SubnetNodesData2::<Test>::try_get(subnet_id, hotkey_subnet_node_id).unwrap();
    ...
    let subnet_node_account = SubnetNodeAccount2::<Test>::get(subnet_id, peer(n));
...
fn test_register_subnet_node() {
  new_test_ext().execute_with(|| {
    ...
    let subnet_node = SubnetNodesData2::<Test>::get(subnet_id, hotkey_subnet_node_id);
    ...
    let subnet_node_account = SubnetNodeAccount2::<Test>::get(subnet_id, peer(total_subnet_nodes+1));
    assert_eq!(subnet_node_account, hotkey_subnet_node_id);
...
fn test_update_coldkey() {
  new_test_ext().execute_with(|| {
    ...
    let hotkey_subnet_node_id = HotkeySubnetNodeId::<Test>::get(subnet_id, account(0)).unwrap();
    let starting_account_subnet_stake = AccountSubnetStake::<Test>::get(account(0), subnet_id);
    ...
    let subnet_node_id_hotkey = SubnetNodeIdHotkey::<Test>::get(subnet_id, hotkey_subnet_node_id).unwrap();
    assert_eq!(subnet_node_id_hotkey, account(0));

    let subnet_node_data = SubnetNodesData2::<Test>::try_get(subnet_id, hotkey_subnet_node_id).unwrap();
    assert_eq!(subnet_node_data.hotkey, account(0));
...
fn test_update_hotkey() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(key_owner, account(0));

    let account_subnet_stake = AccountSubnetStake::<Test>::get(account(0), subnet_id);
...
// #[test]
// fn test_deactivat_subnet_node_and_reactivate() {
//   new_test_ext().execute_with(|| {
//     let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
//     let deposit_amount: u128 = 10000000000000000000000;
//     let amount: u128 = 1000000000000000000000;
...
//     SubnetRewardsValidator::<Test>::insert(subnet_id, epoch as u32, account(1));
//     let validator = SubnetRewardsValidator::<Test>::get(subnet_id, epoch as u32);
//     assert!(validator != None, "Validator is None");
...
//     let subnet_node_data = SubnetNodesData::<Test>::try_get(subnet_id, account(total_subnet_nodes+1)).unwrap();
//     assert_eq!(subnet_node_data.hotkey, account(total_subnet_nodes+1));
//   });
// }
