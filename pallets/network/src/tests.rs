...
//     // let subnet_node = SubnetNodesData::<Test>::get(subnet_id, validator.clone().unwrap());
//     // let before_slash_reputation = subnet_node.reputation;
...
//     // let subnet_node = SubnetNodesData::<Test>::get(subnet_id, validator.clone().unwrap());
//     // assert!(before_slash_reputation > subnet_node.reputation, "Validator reputation was not decreased");
...
//     let before_slash_reputation = subnet_node.reputation;
//     assert_eq!(before_slash_reputation, 1000);
...
#[test]
fn test_validator_reputation_decrease() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
    let deposit_amount: u128 = 10000000000000000000000;
    let amount: u128 = 1000000000000000000000;
...
#[test]
fn test_validator_reputation_decrease_to_removal() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
    let deposit_amount: u128 = 10000000000000000000000;
    let amount: u128 = 1000000000000000000000;
    ...
    let subnet_node = SubnetNodesData::<Test>::get(subnet_id, account(0));
    let mut reputation = subnet_node.reputation;
    assert_eq!(reputation, 1000);

    // give it only 100 epochs for testing and until removed
    for n in 0..100 {
      increase_epochs(1);
      let epoch_length = EpochLength::get();
      let epoch = System::block_number() / epoch_length;
      ...
      let subnet_node = SubnetNodesData::<Test>::try_get(subnet_id, account(0));

      if subnet_node.is_ok() {
        assert!(reputation > subnet_node.clone().unwrap().reputation, "Validator reputation was not decreased");
        reputation = subnet_node.clone().unwrap().reputation;
      } else {
        removed = true;
        break
      }
    ...
    assert!(removed, "Validator not removed");

  });
}

#[test]
fn test_validator_reputation_increase() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
    let deposit_amount: u128 = 10000000000000000000000;
    let amount: u128 = 1000000000000000000000;
    ...
    let subnet_node = SubnetNodesData::<Test>::get(subnet_id, account(0));
    let after_slash_reputation = subnet_node.reputation;
    assert!(before_slash_reputation > after_slash_reputation, "Validator reputation was not decreased");
    ...
    let subnet_node = SubnetNodesData::<Test>::get(subnet_id, account(0));
    log::error!("subnet_node.reputation {:?}", subnet_node.reputation);
    assert!(after_slash_reputation < subnet_node.reputation, "Validator reputation was not increased");
  });
}
...
//     let included_nodes: BTreeSet<<Test as frame_system::Config>::AccountId> = Network::get_classified_accounts(subnet_id, &SubnetNodeClass::Included, epoch);
...
//       let submittable_nodes: BTreeSet<<Test as frame_system::Config>::AccountId> = Network::get_classified_accounts(subnet_id, &SubnetNodeClass::Submittable, epoch);
...
//         );
//         // activated as Idle
//         let subnet_node = SubnetNodesData::<Test>::get(subnet_id, account(n_peers));
//         assert_eq!(subnet_node.classification.class, SubnetNodeClass::Idle);    
//       } else if n == 1 {
...
//       } else if n == 2 {
//         // automatically upgraded to Submittable after first next epoch
//         let subnet_node = SubnetNodesData::<Test>::get(subnet_id, account(n_peers));
