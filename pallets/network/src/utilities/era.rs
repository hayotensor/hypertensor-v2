...
impl<T: Config> Pallet<T> {
  ...
  pub fn get_current_epoch_as_u32() -> u32 {
    ...
    let epoch_length: u32 = T::EpochLength::get() as u32;
  ...
  pub fn do_epoch_preliminaries(block: u32, epoch: u32, epoch_length: u32) {
    ...
    let subnet_activation_enactment_blocks = SubnetActivationEnactmentBlocks::<T>::get();
    ...
    for (subnet_id, data) in subnets {
      ...
      // ==========================
      let max_registration_block = data.registered.saturating_add(data.registration_blocks);
      let max_enactment_block = max_registration_block.saturating_add(subnet_activation_enactment_blocks);
  ...
  pub fn do_epoch_preliminaries_v2(block: u32, epoch: u32, epoch_length: u32) {
    ...
    for (subnet_id, data) in subnets {
      ...
      if activated_epoch == 0 && epoch <= max_registration_epoch {
        // --- Registration Period
        // If in registration period, do nothing
        continue
      } else if activated_epoch == 0 && epoch <= max_enactment_epoch {
        // --- Enactment Period
        // If in enactment period, ensure min nodes
        let subnet_node_ids: Vec<u32> = Self::get_classified_subnet_node_ids(subnet_id, &SubnetNodeClass::Validator, epoch);
        let subnet_nodes_count = subnet_node_ids.len();  
