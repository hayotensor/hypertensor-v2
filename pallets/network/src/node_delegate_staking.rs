...
impl<T: Config> Pallet<T> {
  pub fn do_add_node_delegate_stake(
    ...
    let account_node_delegate_stake_shares = AccountNodeDelegateStakeShares::<T>::get((&account_id, subnet_id, node_account_id.clone()));
    let total_node_delegated_stake_shares = TotalNodeDelegateStakeShares::<T>::get(subnet_id, node_account_id.clone());
    let total_node_delegated_stake_balance = TotalNodeDelegateStakeBalance::<T>::get(subnet_id, node_account_id.clone());

    // --- Get accounts current balance
    let account_delegate_stake_balance = Self::convert_delegate_node_shares_to_balance(
    ...
    if total_node_delegated_stake_shares == 0 {
      ...
      TotalNodeDelegateStakeShares::<T>::mutate(subnet_id, node_account_id.clone(), |mut n| *n += 1000);
  ...
  pub fn do_remove_node_delegate_stake(
    ...
    // --- Get accounts current balance
    let delegate_stake_to_be_removed = Self::convert_delegate_node_shares_to_balance(
      account_node_delegate_stake_shares,
      total_node_delegated_stake_shares,
      total_node_delegated_stake_balance
    );
  ...
  pub fn do_switch_node_delegate_stake(
    ...
    let from_account_delegate_stake_shares: u128 = AccountNodeDelegateStakeShares::<T>::get((&account_id.clone(), from_subnet_id, from_node_account_id.clone()));
    ...
    let total_from_subnet_delegated_stake_shares = TotalNodeDelegateStakeShares::<T>::get(from_subnet_id, from_node_account_id.clone());
    let total_from_subnet_delegated_stake_balance = TotalNodeDelegateStakeBalance::<T>::get(from_subnet_id, from_node_account_id.clone());

    // --- Get accounts current balance
    let delegate_stake_to_be_transferred = Self::convert_delegate_node_shares_to_balance(
    ...
    // --- Add
    let to_account_delegate_stake_shares: u128 = AccountNodeDelegateStakeShares::<T>::get((&account_id.clone(), to_subnet_id, to_node_account_id.clone()));
    let total_to_subnet_delegated_stake_shares = TotalNodeDelegateStakeShares::<T>::get(to_subnet_id, to_node_account_id.clone());
    let total_to_subnet_delegated_stake_balance = TotalNodeDelegateStakeBalance::<T>::get(to_subnet_id, to_node_account_id.clone());

    // --- Get accounts current balance
    let to_account_delegate_stake_balance = Self::convert_delegate_node_shares_to_balance(
    ...
    if total_to_subnet_delegated_stake_shares == 0 {
      // no need for saturation here
      TotalNodeDelegateStakeShares::<T>::mutate(to_subnet_id, to_node_account_id.clone(), |mut n| *n += 1000);
      delegate_stake_to_be_added_as_shares = delegate_stake_to_be_added_as_shares.saturating_sub(1000);
    }
    ...
    Self::deposit_event(Event::DelegateNodeStakeSwitched { 
			...
			from_node_account_id: from_node_account_id, 
			to_subnet_id: to_subnet_id, 
			to_node_account_id: to_node_account_id, 
  ...
  pub fn add_balance_to_node_delegate_stake_unbonding_ledger(
    ...
    if unbondings.len() as u32 == T::MaxDelegateStakeUnlockings::get() {
      // Self::do_claim_node_delegate_stake_unbondings(&account_id, subnet_id);
    }
  ...
  // Infallible
  pub fn do_claim_node_delegate_stake_unbondings(account_id: &T::AccountId, subnet_id: u32) -> u32 {
    let block: u64 = Self::get_current_block_as_u64();
    let epoch_length: u64 = T::EpochLength::get();
    let epoch: u64 = block / epoch_length;
    let unbondings = DelegateStakeUnbondingLedger::<T>::get(account_id.clone(), subnet_id);
  ...
  pub fn increase_account_node_delegate_stake_shares(
    ...
    AccountNodeDelegateStakeShares::<T>::mutate((account_id, subnet_id, node_account_id.clone()), |mut n| n.saturating_accrue(shares));

    // -- increase total subnet delegate stake balance
    TotalNodeDelegateStakeBalance::<T>::mutate(subnet_id, node_account_id.clone(), |mut n| n.saturating_accrue(amount));

    // -- increase total subnet delegate stake shares
    TotalNodeDelegateStakeShares::<T>::mutate(subnet_id, node_account_id.clone(), |mut n| n.saturating_accrue(shares));
  }
  ...
  pub fn decrease_account_node_delegate_stake_shares(
    ...
    AccountNodeDelegateStakeShares::<T>::mutate((account_id, subnet_id, node_account_id.clone()), |mut n| n.saturating_reduce(shares));

    // -- decrease total subnet delegate stake balance
    TotalNodeDelegateStakeBalance::<T>::mutate(subnet_id, node_account_id.clone(), |mut n| n.saturating_reduce(amount));

    // -- decrease total subnet delegate stake shares
    TotalNodeDelegateStakeShares::<T>::mutate(subnet_id, node_account_id.clone(), |mut n| n.saturating_reduce(shares));
  }
  ...
  pub fn do_increase_node_delegate_stake(
    ...
    TotalNodeDelegateStakeBalance::<T>::mutate(subnet_id, node_account_id, |mut n| n.saturating_accrue(amount));
  }

  pub fn convert_delegate_node_shares_to_balance(
    shares: u128,
    total_shares: u128,
    total_balance: u128
  ) -> u128 {
  ...
  pub fn convert_delegate_node_balance_to_shares(
    balance: u128,
    total_shares: u128,
    total_balance: u128
  ) -> u128 {
