...
impl<T: Config> Pallet<T> {
  ...
  //       let subnet_node: Vec<T::AccountId> = Self::get_classified_hotkeys(subnet_id, &SubnetNodeClass::Validator, epoch as u64);
  ...
  //         let hotkey: T::AccountId = subnet_node.hotkey;
  ...
  //             Self::perform_remove_subnet_node(block, subnet_id, hotkey);
  //           }
  //           continue
  ...
  //         let penalties = SubnetNodePenalties::<T>::get(subnet_id, hotkey.clone());
  ...
  //           SubnetNodePenalties::<T>::insert(subnet_id, hotkey.clone(), penalties + 1);
  ...
  //               Self::perform_remove_subnet_node(block, subnet_id, hotkey.clone());
  //             }
  //           }
  ...
  //           SubnetNodePenalties::<T>::mutate(subnet_id, hotkey.clone(), |n: &mut u32| n.saturating_dec());
  //           continue
  //         }
  ...
  //         if !submission.attests.contains_key(&hotkey) {
  //           if attestation_percentage > min_vast_majority_attestation_percentage {
  ...
  pub fn reward_subnets(block: u64, epoch: u32) -> DispatchResultWithPostInfo {
    ...
    for (subnet_id, data) in SubnetsData::<T>::iter() {
      ...
      if let Ok(mut submission) = SubnetRewardsSubmission::<T>::try_get(subnet_id, epoch) {
        ...
        // --- Reward validators
        for (hotkey, subnet_node) in SubnetNodesData::<T>::iter_prefix(subnet_id) {
          // --- (if) Check if subnet node is past the max registration epochs to activate (if registered or deactivated)
          // --- (else if) Check if past Idle and can be included in validation data
          // Always continue if any of these are true
          // Note: Only ``included`` or above nodes can get emissions
